...
export async function GET(request: NextRequest) {
  try {
    ...
    const addOnLines: { name: string; price: number; status: string }[] = addOnIds.map((aid) => {
      ...
      if (!addOn) return { name: aid, price: 0, status: "" };
      const price = addOn.perDay ? addOn.price * days : addOn.price;
      const status = isPayAtCollection ? "to be paid on collection" : "included in payment above";
      return { name: addOn.name, price, status };
    });
    ...
    let y = 842 - TOP_MARGIN; // start below top margin for printing
    ...
    if (addOnLines.length === 0) {
      ...
      for (const line of addOnLines) {
        const text = "  - " + line.name + ": P" + line.price + " - " + line.status;
    ...
    draw("   Any damages not present in the video will be considered the renter's responsibility.");
